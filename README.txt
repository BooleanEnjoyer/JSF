...
Program selects order with highest value posible among others.
...
Variables:
gson - builds a reader to read order from Json
gsonS - builds a reader to read store from Json
OrderType - maps List of order to Type
Store_reader - path to read store json file
Order_reader - path to read order json file
jsonStore - readed store from json
jsonOrderList - readed order from json
jsonOrderList.sort(new OrderComparator()); - this line sorts by time order should be pick to complete it
...
CompleatedOrders - list of order that have been compleated
...
Orders - class that represent order object readed from json
